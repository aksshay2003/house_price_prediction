...
class SimpleMissingValuesAnalysis(MissingValuesAnalysisTemplate):
    ...
    def visualize_missing_values(self, df: pd.DataFrame):
        ...
        '''
        df.isnull() → Creates a DataFrame of the same shape as df, where each value is True (if the original value is missing) or False (if the original value is not missing).
        cbar=False → Disables the color bar (the legend that usually shows the range of values).
        purple=no missing values
        yellow=misssing values
        '''
